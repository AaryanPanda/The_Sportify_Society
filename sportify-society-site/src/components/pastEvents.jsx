      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 scroll-reveal">
          {pastEvents.slice(0, 2).map((event, index) => {
            // Trim description to 120 chars for uniformity
            const trimmedDesc = event.description.length > 120
              ? event.description.slice(0, 120) + "..."
              : event.description;
            return (
              <div
                key={event.id}
                className="bg-gradient-to-br from-gray-800 via-gray-900 to-black rounded-xl overflow-hidden shadow-lg flex flex-col md:flex-row transform hover:scale-[1.02] transition-all duration-300 gap-2 md:gap-0"
              ...
              >
                ...
                {/* Content */}
                <div className="md:w-3/5 p-7 flex flex-col justify-center gap-3">
                  {/* Title + Date */}
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-lg font-bold text-white">{event.title}</h3>
                    <span className="text-xs text-white px-2 py-1 rounded bg-gradient-to-r from-[#ff4500]/20 via-[#ff6a00]/30 to-[#ffce00]/40">
                      {event.date}
                    </span>
                  </div>
